...
max([U,V|T],M) :- max2(U,V,M1), max([M1|T],M2), M is M2.

% Somme liste
somme([A,B],S) :- sum(A,B,S), !.
somme([U,V|T],S) :- sum(U,V,S1), somme([S1|T],S2), S is S2.
...
% Monnaie
rend_monnaie(Argent,Prix) :- (Argent-Prix)<(0.05). % Evite les erreurs d'entrées
...
rend_monnaie(Argent,Prix) :- A1 is Argent+(0.001), R is (A1-Prix), R>=1, M is floor(R/1), N is A1-1*M, (M==0;(write(M), write(' piece de 1'), nl)), rend_monnaie(N,Prix).
