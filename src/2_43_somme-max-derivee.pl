...
d(F,X,G) :- F=A+B, B\==X, ((B\=X^_,B\=_*X,B\=_*X^_,B\=_+_);atomic(B)), d(A,X,G), !. % Appel récursif pour polynôme, deuxième terme = constante
d(F,X,G) :- F=A+B, A\==X, ((A\=X^_,A\=_*X,A\=_*X^_,A\=_+_);atomic(A)), d(B,X,G), !. % Appel récursif pour polynôme, premier terme = constante
